 ...
 * @author ijazfx
 */
public class VroomJSCompletionQuery extends AsyncCompletionQuery {
    ...
    protected void query(CompletionResultSet rs, Document doc, int caretOffset) {
        ...
        if (uri != null) {
            listItems = ctx.getHtmlIdList(uri, true);
            for (ListItem item : listItems) {
                items.add(new VroomEditorCompletionItem(item.getText(), item.getDescription(), Constants.ID_ICON, 2000));
            }
        }
        if(cssFiles != null) {
            listItems = ctx.getCssItemList(cssFiles, true);
            for (ListItem item : listItems) {
                items.add(new VroomEditorCompletionItem(item.getText(), item.getDescription(), Constants.ATTRIBUTE_ICON, 3000));
            }
        }
        if(jsFiles != null) {
            listItems = ctx.getJsFunctionList(jsFiles, true);
            for (ListItem item : listItems) {
                items.add(new VroomEditorCompletionItem(item.getText(), item.getDescription(), Constants.METHOD_ICON, 4000));
            }
        }
        if(items.size() > 0) {
